...
class CalculadorDesgloseAnual:
    ...
    def _construir_items_detalle(self, d: DesgloseNominaAnual, liq: LiquidacionNomina) -> list[ItemDesgloseAnual]:
        ...
        acuerdo_pos = "Acuerdo 027 (UPC)" if "CESAR" in d.nombre_universidad.upper() or d.id_universidad == 1 else "Normativa Interna"
        if d.bonificaciones_posgrado_anual > 0:
            items.append(ItemDesgloseAnual(
                concepto=f"Bonificación por Posgrado ({acuerdo_pos})",
                categoria="DEVENGADO",
    ...
    def generar_desglose_institucional(
        self,
        anio: int = 2026,
        id_universidad: int | None = None,
    ) -> dict[str, Any]:
        """Calcula el consolidado anual para una universidad específica o para todas."""
